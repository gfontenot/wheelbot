function doConfigure( filePathAndName ) {
	...
	document.location = "ACCS_Sample_Config.php?scratchListId=" + scratchListId + "&filePathAndName=" + filePathAndName;
}
...
function toggleOption(optionCode) {
    ...
    request.open("get", "ACCS_Sample_Config_ToggleOption.php?optionCode=" + optionCode + "&random=" + new Date(), false);
    request.send(null);
...
function saveStyle( styleName ) {
	var request = getRequest();
    request.open("get", "ACCS_Sample_SaveStyle.php?fileName=" + styleName + "&random=" + new Date(), false);
    request.send(null);
...
function showColorWindow() {
    colorWindow = window.open( "ACCS_Sample_Config_Colors.php", "colorWindow", "width=500px, height=600px, toolbar=no, menubar=no,scrollbars=yes,resizable=yes");
}
// -->