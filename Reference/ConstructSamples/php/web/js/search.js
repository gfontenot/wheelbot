...
function getSearchCriteria()
{
    ...
    request.open("get", "ACCS_Sample_Search_Data.php?data=getSearchCriteria", false);
    request.send(null);
...
function selectLocale()
{
    ...
    //do request
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.php?data=locale&locale=" + locale, false);
    request.send(null);
...
function selectMode()
{
    ...
    //do request
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.php?data=orderAvailability&orderAvailability=" + mode, false);
    request.send(null);
...
function criterionSelected(){
    ...
    if( descriptor )
    {
        ...
        if( type == "NumberRange" || type == "MoneyRange" || type == "TechnicalSpecificationRange" ){
            setElementText( minField, descriptor.min );
            setElementText( maxField, descriptor.max );
            document.getElementById( minValueField ).value = "";
            document.getElementById( maxValueField ).value = "";
            // document.getElementById( minValueField ).onblur = function(){validateValue( type, document.getElementById( minValueField ), descriptor.min, descriptor.max, true ) }
            // document.getElementById( maxValueField ).onblur = function(){validateValue( type, document.getElementById( maxValueField ), descriptor.min, descriptor.max, true ) }
        } else if ( type == "String" ){
...
function getOptionalValues( stringTokenName ){
    ...
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.php?data=getOptionalValues&tokenName=" + stringTokenName, false);
    request.send(null);
...
function doSearch()
{
    ...
    var requestString = "ACCS_Sample_Search_Data.php?data=getSearchResults";
    ...
    updateStatusMessage( "Retrieving Data...Please Wait", "green" );
...
function addToScratchList( styleName ) {
    ...
    var request = getRequest();
    request.open("get", "ACCS_ScratchList.php?cmd=add&styleId=" + styleId, false);
    request.send(null);

    var result = request.responseText.split("~~")[0];
    var resultValue = request.responseText.split("~~")[1];
...
function removeScratchListRow() {
    ...
    for( var i = rows.length - 1; i >= 0; --i )	{
		if( checkBoxes[i].checked ){
            request.open("get", "ACCS_ScratchList.php?cmd=remove&scratchListId=" + checkBoxes[i].value, false);
            request.send(null);
...
function removeScratchListAll() {
    var request = getRequest();
    request.open("get", "ACCS_ScratchList.php?cmd=clear", false);
    request.send(null);
    var result = request.responseText;
    if( result ==  "success" ){
        var table = document.getElementById("scratchListTable");
...
function findComparable()
{
    ...
    var requestString = "ACCS_Sample_Search_Data.php?data=findComparable";
    requestString += "&filterTBD=" + filterTBD;
    requestString += "&filterPostalCode=" + filterPostalCode;
    requestString += "&postalCode=" + postalCode;
    requestString += "&maxNumResults=" + maxNumResults;
    ...
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.php?data=getAvailableMakes&scratchListId=" + scratchListId + "&year=" + modelYear, false);
    request.send(null);
