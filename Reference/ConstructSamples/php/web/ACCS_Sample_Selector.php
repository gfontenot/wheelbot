...
<div id="styleDiv" style="visibility:hidden">
	<table id="styleTableHeader" border="0" cellspacing="0" cellpadding="5" width="100%">
		<tr><td width="10%"><b>Styles</b></td><td width="70%">&nbsp;</td><td align="center" width="10%"><b>Invoice</b></td><td align="center" width="10%"><b>MSRP</b></td></tr>
	</table>
...
<?
	...
	if( is_dir( $dir ) ) {
	    if( $handle = opendir( $dir ) ) {
	        while( false !== ( $file = readdir( $handle ) ) ) {
	        	if( ( $file != "." && $file != ".." ) ) {
	                array_push( $savedStyles, basename( $file ) );
				}
	        }
	        closedir( $handle );
	    }
    ...
    //show saved styles
	if ( count( $savedStyles ) > 0 ) {
?>
		<span><b>Saved Styles</b></span><br>
		<table id="savedStyleTable" border="1" cellspacing="0" cellpadding="5" width="100%">
<?
		for( $i = 0; $i < count($savedStyles); $i++ ) {
			$savedStyle = $dir . $savedStyles[$i];
			$savedStyleFileName = substr( $savedStyles[$i], 0, strlen( $savedStyles[$i] ) - 4 );
?>


	<tr>
		<td align="center" width="10%"><input type="button" id="<?=$savedStyle?>" value="Configure" onClick="doConfigure(this.id)"></td>
