function doConfigure( filePathAndName ) {
	...
	document.location = "ACCS_SAMPLE_Config_New.aspx?scratchListId=" + scratchListId + "&filePathAndName=" + filePathAndName;
}
...
function toggleOption(optionCode) {
    ...
    request.open("get", "ACCS_SAMPLE_Config_ToggleOption.aspx?optionCode=" + optionCode + "&random=" + new Date(), false);
    request.send(null);
	...
	if ( isConflict == "yesConflict" ) {
		...
		//show mask
		modalWinMaskDiv.style.height = document.body.offsetHeight + "px";
	    modalWinMaskDiv.style.width = document.documentElement.clientWidth + "px";	    
		...
		conflictDiv.style.top = document.documentElement.scrollTop + 200 + "px";
        conflictDiv.style.left = "200px";
...
function showColorWindow() {
    colorWindow = window.open( "ACCS_SAMPLE_Config_Colors.aspx", "colorWindow", "width=500px, height=600px, toolbar=no, menubar=no,scrollbars=yes,resizable=yes");
}

function saveStyle( styleName ) {
	var request = getRequest();
    request.open("get", "ACCS_SAMPLE_Save_Style.aspx?actionType=add&styleName=" + styleName, false);
    request.send(null);
...
{
    var request = getRequest();
    request.open("get", "ACCS_SAMPLE_Save_Style.aspx?actionType=delete&styleName=" + styleName, false);
    request.send(null);
