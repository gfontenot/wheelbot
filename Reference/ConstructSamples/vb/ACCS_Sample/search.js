...
function getSearchCriteria()
{
    ...
    request.open("get", "ACCS_Sample_Search_Data.aspx?data=getSearchCriteria", false);
    request.send(null);
...
function selectLocale()
{
    ...
    //do request
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.aspx?data=locale&locale=" + locale, false);
    request.send(null);
...
function selectMode()
{
    ...
    //do request
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.aspx?data=orderAvailability&orderAvailability=" + mode, false);
    request.send(null);
...
function getOptionalValues( stringTokenName ){
    ...
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.aspx?data=getOptionalValues&tokenName=" + stringTokenName, false);
    request.send(null);
...
function doSearch()
{
    ...
    var requestString = "ACCS_Sample_Search_Data.aspx?data=getSearchResults";
...
function fillStyleTable(responseText)
{
    ...
    if( responseText.length > 0 )
    {
        ...
        for (var i = 0; i < allStyles.length; i++)
        {
            ...
            td.innerHTML = "<input type='button' value='Add to List' onClick='addToScrachList(this.name)' name='" + eachStyle + "'></>";
...
function addToScrachList( styleName ) {
     ...
     var request = getRequest();
     request.open("get", "ACCS_Sample_ScratchList.aspx?cmd=add&styleId=" + styleId, false);
     request.send(null);
...
function removeScratchListRow() {
     ...
     for( var i = rows.length - 1; i >= 0; --i )	{
         if( checkBoxes[i].checked ){
             request.open("get", "ACCS_Sample_ScratchList.aspx?cmd=remove&sratchListId=" + checkBoxes[i].value, false);
             request.send(null);
...
function findComparable()
{
    ...
    var requestString = "ACCS_Sample_Search_Data.aspx?data=findComparable";
    requestString += "&filterTBD=" + filterTBD;
    requestString += "&filterPostalCode=" + filterPostalCode;
    requestString += "&postalCode=" + postalCode;
    requestString += "&maxNumResults=" + maxNumResults;
    ...
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.aspx?data=getAvailableMakes&scratchListId=" + scratchListId + "&year=" + modelYear, false);
    request.send(null);
