...
function getSearchCriteria()
{
    ...
    request.open("get", "ACCS_Sample_Search_Data.cfm?data=getSearchCriteria", false);
    request.send(null);
...
function selectLocale()
{
    ...
    //do request
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.cfm?data=locale&locale=" + locale, false);
    request.send(null);
...
function selectMode()
{
    ...
    //do request
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.cfm?data=orderAvailability&orderAvailability=" + mode, false);
    request.send(null);
...
function getOptionalValues( stringTokenName ){
    ...
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.cfm?data=getOptionalValues&tokenName=" + stringTokenName, false);
    request.send(null);

    var response = trimString( request.responseText );
    if( response.length > 0 ){
	    var optionalValues = response.split(";;");
	    for( var i=0; i < optionalValues.length; i++ ){
...
function doSearch()
{
    ...
    var requestString = "ACCS_Sample_Search_Data.cfm?data=getSearchResults";
...
function addToScratchList( styleName ) {
    ...
    var result = responseText.split("~~")[0];
    var resultValue = responseText.split("~~")[1];
...
function findComparable()
{
    ...
    var requestString = "ACCS_Sample_Search_Data.cfm?data=findComparable";
    requestString += "&filterTBD=" + filterTBD;
    requestString += "&filterPostalCode=" + filterPostalCode;
    requestString += "&postalCode=" + postalCode;
    requestString += "&maxNumResults=" + maxNumResults;
    ...
    var request = getRequest();
    request.open("get", "ACCS_Sample_Search_Data.cfm?data=getAvailableMakes&scratchListId=" + scratchListId + "&year=" + modelYear, false);
    request.send(null);
